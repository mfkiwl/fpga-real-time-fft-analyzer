...
-- CRC-32 calculator for 8-bit data (Ethernet standard polynomial)
entity crc is
  port (
...
architecture imp_crc of crc is
  signal lfsr_q     : std_logic_vector(31 downto 0) := (others => '1'); -- CRC shift register (starts at all 1s)
  signal lfsr_c     : std_logic_vector(31 downto 0) := (others => '0'); -- next CRC value (combinational)
  signal data_in    : std_logic_vector(7 downto 0)  := (others => '0'); -- bit-flipped input data
  signal crc_enable : std_logic := '0';                                  -- local copy of enable

begin
  crc_out <= lfsr_q; -- output current CRC state

  crc_enable <= crc_en; -- mirror for debug visibility

  -- Flip input byte bit order (MSB becomes LSB, etc.)
  ...
  -- Parallel CRC-32 computation for all 32 bits (processes entire byte at once)
  -- These equations implement the standard Ethernet CRC polynomial in parallel
  ...
  -- Update CRC register on clock edge
  process (clk)
  begin
    if rising_edge(clk) then
      if (rst = '0') or (init = '1') then
        lfsr_q <= (others => '1');         -- reset to all 1s (CRC initial state)
      else
        if (crc_en = '1') then
          lfsr_q <= lfsr_c;                -- update with new CRC value
        end if;
      end if;
    end if;
  end process;
