library IEEE;
use IEEE.STD_LOGIC_1164.ALL;
use IEEE.NUMERIC_STD.ALL;

-- True dual-port RAM - both ports can read/write independently
entity dpram is
  generic (
    G_ADDR_WIDTH : positive := 11;  -- Address bus width (depth = 2**G_ADDR_WIDTH)
    G_DATA_WIDTH : positive := 8    -- Data word width
  );
  port (
    -- Port A
    i_clk_a   : in  std_logic;                                  -- Clock A
    i_en_a    : in  std_logic;                                  -- Enable A
    i_we_a    : in  std_logic;                                  -- Write enable A
    i_addr_a  : in  std_logic_vector(G_ADDR_WIDTH-1 downto 0);  -- Address A
    i_write_a : in  std_logic_vector(G_DATA_WIDTH-1 downto 0);  -- Write data A
    o_read_a  : out std_logic_vector(G_DATA_WIDTH-1 downto 0);  -- Read data A

    -- Port B
    i_clk_b   : in  std_logic;                                  -- Clock B
    i_en_b    : in  std_logic;                                  -- Enable B
    i_we_b    : in  std_logic;                                  -- Write enable B
    i_addr_b  : in  std_logic_vector(G_ADDR_WIDTH-1 downto 0);  -- Address B
    i_write_b : in  std_logic_vector(G_DATA_WIDTH-1 downto 0);  -- Write data B
    o_read_b  : out std_logic_vector(G_DATA_WIDTH-1 downto 0)   -- Read data B
  );
end entity dpram;

architecture rtl of dpram is

  constant RAM_DEPTH : positive := 2**G_ADDR_WIDTH - 1;  -- max address index
  
  -- Main memory array
  type ram_t is array (0 to RAM_DEPTH) of std_logic_vector(G_DATA_WIDTH-1 downto 0);
  signal ram      : ram_t;                                      -- the actual RAM storage
  
  -- Registered addresses for reads (pipelined)
  signal r_addr_a : std_logic_vector(G_ADDR_WIDTH-1 downto 0);  -- latched read address A
  signal r_addr_b : std_logic_vector(G_ADDR_WIDTH-1 downto 0);  -- latched read address B

begin

  -- Async read outputs using registered addresses
  o_read_a <= ram(to_integer(unsigned(r_addr_a)));
  o_read_b <= ram(to_integer(unsigned(r_addr_b)));

  -- Dual-clock process handles both ports independently  
  p_dpram: process(i_clk_a, i_clk_b)
  begin
    -- Port A: latch address and write if enabled
    if rising_edge(i_clk_a) then
      if i_en_a = '1' then
        r_addr_a <= i_addr_a;  -- register address for next read
        if i_we_a = '1' then
          ram(to_integer(unsigned(i_addr_a))) <= i_write_a;  -- write data
        end if;
      end if;
    end if;

    -- Port B: same operations, independent clock
    if rising_edge(i_clk_b) then
      if i_en_b = '1' then
        r_addr_b <= i_addr_b;  -- register address for next read
        if i_we_b = '1' then
          ram(to_integer(unsigned(i_addr_b))) <= i_write_b;  -- write data
        end if;
      end if;
    end if;
  end process p_dpram;

end architecture rtl;
