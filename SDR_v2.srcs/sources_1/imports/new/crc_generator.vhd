...
-- CRC-32 generator with final output formatting (inverted + bit-reversed)
entity crc_gen is
  port (
    clk     : in  std_logic;
...
architecture rtl of crc_gen is
  signal lfsr_q  : std_logic_vector(31 downto 0) := (others => '1'); -- CRC shift register (starts all 1s)
  signal lfsr_c  : std_logic_vector(31 downto 0);                    -- next CRC value (combinational)
...
begin
  ...
  -- Parallel CRC-32 computation equations (processes entire byte at once)
  ...
  begin
    if rising_edge(clk) then
      if rst = '0' then
        lfsr_q <= (others => '1');  -- reset to all 1s
      else
        if init = '1' then
          lfsr_q <= (others => '1');  -- sync init to all 1s
        elsif crc_en = '1' then
          lfsr_q <= lfsr_c;  -- advance CRC with new data
        end if;
      end if;
    end if;
  end process;

  -- Output final CRC: invert all bits and reverse bit order (Ethernet standard format)
  gen_final : for i in 0 to 31 generate
    crc_out(i) <= not lfsr_q(31 - i);  -- bit reverse + invert
  end generate;
