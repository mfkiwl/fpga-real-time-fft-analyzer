...
-- Configurable IIR filter (Alpha or Beta type) using fixed coefficients from package
entity filter_iir is
  ...
  port (
    ...
    o_data      : out std_logic_vector(15 downto 0)        -- Filtered output data
  );
...
begin
  ...
  -- Alpha filter coefficients (from package constants)
  g_alpha: if G_FILTER_TYPE = ALPHA generate
    mult_b0 <= ve(2) * COEFF_IIR_ALPHA_B0;  -- B0 * x[n-2]
    mult_b1 <= ve(1) * COEFF_IIR_ALPHA_B1;  -- B1 * x[n-1]
    mult_b2 <= ve(0) * COEFF_IIR_ALPHA_B2;  -- B2 * x[n]
    mult_a0 <= vs(2) * COEFF_IIR_ALPHA_A0;  -- A0 * y[n-2]
    mult_a1 <= vs(1) * COEFF_IIR_ALPHA_A1;  -- A1 * y[n-1]
  end generate g_alpha;

  -- Beta filter coefficients (from package constants)
  g_beta : if G_FILTER_TYPE = BETA generate
    mult_b0 <= ve(2) * COEFF_IIR_BETA_B0;   -- B0 * x[n-2]
    mult_b1 <= ve(1) * COEFF_IIR_BETA_B1;   -- B1 * x[n-1]
    mult_b2 <= ve(0) * COEFF_IIR_BETA_B2;   -- B2 * x[n]
    mult_a0 <= vs(2) * COEFF_IIR_BETA_A0;   -- A0 * y[n-2]
    mult_a1 <= vs(1) * COEFF_IIR_BETA_A1;   -- A1 * y[n-1]
  end generate g_beta;
